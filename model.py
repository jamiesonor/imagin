...
class ModelIMAGIN(nn.Module):
    def __init__(self, input_dims, hidden_dims, num_classes, num_layers, sparsities, dropout=0.5, cls_token='sum', readout='sero'):
        ...
        self.linear = nn.Linear(hidden_dims[0]+hidden_dims[1]+hidden_dims[2] + 19, num_classes)
        self.dropout = nn.Dropout(dropout)

    def forward(self, v, a, t, sampling_endpoints, phenotypes):
        ...
        logit = self.dropout(self.linear(torch.cat([latent_aal, latent_cc200, latent_schaefer, phenotypes], dim=2)))
